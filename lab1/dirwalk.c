...
char * check_for_path(int argc, char * argv[]){ //проверка на путь в аргументах
    ...
    printf("No path entered, yet.\n"); // завершение программы при отсутсвии пути
    return NULL;
}
...
void dir_walking(char * path, int * opt_flags){ //рекурсивный обход
    ...
    struct dirent * entry;
    while((entry = readdir(dir)) != NULL){ //home/seno/.
        if(char_equals(entry->d_name, ".") || char_equals(entry->d_name, "..")){
            continue; //фильтр на текущую директорию и родителя, чтобы избежать бесконечную рекурсию
        }
        ...
        if(lstat(newPath, &info) == 0){
            ...
            else if(S_ISLNK(info.st_mode) && opt_flags[0] == 1){ // link
                paths[paths_count++] = newPath;
            }
            else if(S_ISREG(info.st_mode) && opt_flags[2] == 1){ //file
                paths[paths_count++] = newPath;
            }
        }
    }
